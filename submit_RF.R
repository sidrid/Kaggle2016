...
table(test$IsRep, PredictForest>.5)
submission2 = data.frame(USER_ID = test$USER_ID, Predictions = as.factor(ifelse(PredictForest >.5 ,"Republican","Democrat")))
write.csv(submission2, "submission_log1.csv", row.names=FALSE) 
...
vars = setdiff(names(train), c("IsRep",  "YOB", "Gender", "Party","USER_ID","HouseholdStatus","EducationLevel","Income"))
cor(train[vars])
